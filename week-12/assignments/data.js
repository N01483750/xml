  ...
  if (withFilters) {
    ...
    switch(filterName) {
      ...
      case 'state':
              ...
              break;
      case 'postal_code':
              data = data.filter(({postal_code }) => postal_code.toLowerCase().includes(valueTerm));
              break;
      case 'university':
              data = data.filter(({ university }) => university.toLowerCase().includes(valueTerm));
              break;
    }
  
  }

  const rows = data.map(({ id, first_name, last_name, email, gender, ip_address,contact_number, city, state, postal_code, university }) =>
    table.appendChild(
      htmlToElement(
        `<tr id="table-row-${id}">
          <td>${id}</td>
