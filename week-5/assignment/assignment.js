...
const renderTable = () => {
    ...
    if (withFilters) {
      ...
      switch(filterName) {
        case 'fname':
            data = data.filter(({ first_name }) => first_name.toLowerCase().includes(value));
            break;
        case 'lname':
            data = data.filter(({ last_name }) => last_name.toLowerCase().includes(value));
            break;
        case 'email':
            data = data.filter(({ email }) => email.toLowerCase().includes(value));
            break;
        case 'gender':
            data = data.filter(({ gender }) => gender.toLowerCase().includes(value));
            break;
        case 'ip_address':
            data = data.filter(({ ip_address }) => ip_address.toLowerCase().includes(value));
            break;
      }
